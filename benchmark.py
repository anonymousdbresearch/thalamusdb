...
def queries_craigslist():
    ...
    col2conditions = {
        'img': ['blue chair'],  # ['blue chair', 'table', 'wooden'],
...
def main():
    ...
    ground_truths = {'blue chair': 21.63, 'table': 20.57, 'wooden': 16.02,  # craigslist
                     'good condition': 0.3201, 'wood': 0.351764,  # craigslist
                     'voices': -32.8471, 'cooking': 0.28863, 'driving': 0.2256}  # youtubeaudios
    constraints = [('error', 0.1, 1), ('error', 0.1, 10000)]  # ('runtime', 10, 10), ('feedback', 5, 100), ('error', 0.1, 10)  # (constrained metric, value, ratio versus runtime)
    methods = ['local', 'naive']  # ['local', ('cost', 1), ('cost', 2), ('cost', 5), 'random', 'ordered', 'naive']
    use_cache = False
    ...
    #     "select avg(price) from images, furniture where (nl(img, 'blue chair') and nl(title_u, 'good condition')) and images.aid = furniture.aid"
    ...
    #     "select max(length) from youtube where nl(audio, 'voices')",
