...
class AudioPrecomputedDataset(Dataset):
    def __init__(self, name='AudioCaps'):
        """
        Load audio clip's waveform.
        Args:
            name: 'AudioCaps', 'Clotho
        """
        super(AudioPrecomputedDataset, self).__init__()
        self.name = name
        self.h5_path = f'../audio-text_retrieval/data/{name}/hdf5s/train/train.h5'
    ...
    def __getitem__(self, idx):
        audio_name = self.audio_keys[idx]
        with h5py.File(self.h5_path, 'r') as hf:
            waveform = hf['waveform'][idx]
        return waveform, audio_name, idx, len(waveform)
