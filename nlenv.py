...
class NLEnv(gym.Env):
    ...
    def __init__(self, nldb, query, error_constraint, nl_filters, max_nr_actions=10):
        ...
        self.cur_step = 0
        self.recreate_scores_table()
        self.prev_error = self.query_to_compute_error()  # Needs to be after re-creating the scores table
    ...
    def step(self, action):
        ...
        if option[0] == 'i' or option[0] == 'o':
            ...
            if option[0] == 'i':
                ...
                table_name = self.query.get_table_by_col(col_name, self.nldb)
        ...
        # Reward computation.
        error = self.query_to_compute_error()
    ...
    def render(self, mode='console'):
        ...
        error = self.query_to_compute_error(print_error=True)
        return error

    def error_and_query_result(self):
        error, result_l, result_u = self.query_to_compute_error(print_error=True, return_results=True)
        return error, result_l, result_u
    ...
    def query_to_compute_error(self, print_error=False, print_result=False, return_results=False):
        ...
        if return_results:
            return error_avg, result_l, result_u
