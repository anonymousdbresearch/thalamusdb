...
def phase1_wrapper(_db, sql, optimizer_mode, constraint):
...
def draw_line_bounds():
    ...
    if lus:
        if len(lus) == 1:
            st.session_state["lus"].append(lus[0])
        lus = st.session_state["lus"]
...
def process(db, sql, constraint, optimizer_mode):
    phase1_wrapper(db, sql, optimizer_mode, constraint)  # Cached.
    ...
    if st.session_state["label_idx"] < total_nr_feedbacks:
        ...
        if not (is_error_constraint and error <= error_constraint) and \
            ...
            if not action_queue:
                if optimizer_mode == 'local':
                    query_info = NLQueryInfo(db.query, db.nldb)
            ...
            if action_queue:
                ...
                st.session_state["execution_plan"].append(action)
        ...
        # When error below the given constraint.
            ...
            with st.expander("Similarity Score Distribution for NL Predicates"): # expanded=True
                for nl_filter in nl_filters:
                    filter_name = str(nl_filter)
                    chart_data = pd.DataFrame(
                        np.fromiter(nl_filter.idx_to_score.values(), float),
                        columns=[filter_name])
                    nr_bins = 10
                    hist = alt.Chart(chart_data, title=f"{filter_name} - Score thresholds: [{nl_filter._lower:.3f}, {nl_filter._upper:.3f}]").mark_bar().encode(
                        x = alt.X(filter_name, bin = alt.BinParams(maxbins = nr_bins)), y = 'count()')
                    st.altair_chart(hist, use_container_width=True)

            with st.expander("Execution Plan"): # expanded=True
                actions = st.session_state["execution_plan"]
                errors = st.session_state["errors"][-len(actions):]
                for idx, action in enumerate(actions):
                    cnt = idx + 1
                    fid = action[-1]
                    if action[0] == 'u':
                        description = f"Labeling request for {nl_filters[fid]}"
                    elif action[0] == 'i':
                        description = f"Process more items for {nl_filters[fid]} via uniform sampling"
                    elif action[0] == 'o':
                        description = f"Process more items for {nl_filters[fid]} prioritized by ({action[3]}, {db.query.cols[action[2]]})"
                    else:
                        raise ValueError(f'Our action is out of scope: {action}')

                    st.write(f"[Error: {errors[idx]:.3f}] Action {cnt}: {description}")

                # st.write(str(st.session_state["execution_plan"]))
...
with st.sidebar:
    ...
    constraint_max = 1.0 if constraint_on == 'Error' else 100.0 if constraint_on == 'Interactions' else 1000.0
