...
class NLFilter:
    ...
    def streamlit_collect_user_feedback_get(self, weight):
        ...
        return self.col.processor.show(idx), idx
    ...
    def nr_unsure(self):
        if len(self.idx_to_score) == 0:
            return 0
        cnt = 0
        for score in self.idx_to_score.values():
            if self.lower < score < self.upper:
                cnt += 1
        return cnt

    def idxs_with_temp_bounds(self, lower, upper):
        idxs_t = {idx for idx, score in self.idx_to_score.items() if score >= upper}
        idxs_f = {idx for idx, score in self.idx_to_score.items() if score <= lower}
        idxs_u = {idx for idx, score in self.idx_to_score.items() if lower < score < upper}
        return FilterResult(idxs_t, idxs_f, idxs_u, self.col.processor.nr_total, self.ordering_to_cnt)

    def estimate_idxs_tfu(self, weight):
        # New threshold.
        cur_score = self.lower * (1 - weight) + self.upper * weight
        # When user answers yes.
        idxs_yes = self.idxs_with_temp_bounds(self.lower, cur_score)
        # When user answers no.
        idxs_no = self.idxs_with_temp_bounds(cur_score, self.upper)
        return idxs_yes, idxs_no
