...
class NLDatabase:
    ...
    def process_unstructured(self, action, query, nl_filters, processed_percent):
        ...
        if action_type == 'i':
            ...
            table_name = query.get_table_by_col(col_name, self)
            ...
            sids = self.con.execute(sql).df().iloc[:, 0]
    ...
    def run(self, query, constraint, ground_truths=None, optimizer_mode='rl', col2cache={}):
        ...
        for fid, nl_filter in enumerate(nl_filters):
            ...
            if nl_filter.idx_to_score:
                self.con.execute(f"INSERT INTO scores{fid} VALUES {', '.join(f'({key}, {val}, TRUE)' for key, val in nl_filter.idx_to_score.items())}")
            ...
            if ordering2cnt is not None:
                for ordering, cnt in ordering2cnt.items():
                    if ordering == ('uniform',) or ordering[1] in query.cols:
                        ...
                        self.process_unstructured(action, query, nl_filters, percent)
        ...
        for fid, nl_filter in enumerate(nl_filters):
            ...
            for ordering in possible_orderings:
                action = ('i', 1, fid) if ordering == ('uniform',) else ('o', 1, query.cols.index(ordering[1]), ordering[0], fid)
                self.process_unstructured(action, query, nl_filters, percent_per_ordering)
        ...
        # Run query via reinforcement learning.
        ...
        while True:
            ...
            if (optimizer_mode == 'local' and not no_improvement) or \
                ...
                if not action_queue:
                    ...
                    if optimizer_mode == 'rl':
                        ...
                        start_rl = time.time()
                        model = A2C('MlpPolicy', env, verbose=0).learn(1000)
                        end_rl = time.time()
                        time_rl = end_rl - start_rl
                        result_info['time_rl'] += time_rl
                        print(f'Time: {time_rl}')
                        time_sql_in_rl = sum(nlenv.runtimes.values())
                        ...
                        print(f'SQL Time - Recreate, Insert, Query, Total: {nlenv.runtimes["recreate"]} {nlenv.runtimes["insert"]} {nlenv.runtimes["query"]} {time_sql_in_rl}')
                ...
                if action_queue:
                    ...
                    if action_type == 'i' or action_type == 'o':  # or action_type == 'c':
                        ...
                        self.process_unstructured(action, query, nl_filters, processed_percent)
                    elif action_type == 'u':
            ...
            # When error below the given constraint.
                ...
                print('==========QUERY RESULT==========')
                error = img_env.query_to_compute_error(print_error=True, print_result=True)
    ...
    # def query(self, query, nl_filters, thresholds):
    #     # Compute lower and upper bounds.
    #     # Lower and upper thresholds of NL predicates are given as a list of tuples, e.g., [(lt, ut), ...]
    #     sql_l, sql_u = query.to_lower_upper_sqls(nl_filters, thresholds)
    #     # if is_print:
    #     #     print(f'sql_l: {sql_l}')
    ...
    #     lus = []
    #     if query.limit < 0:
    #         for idx, (select_str, *_) in enumerate(con_l.description):
    ...
    def run_baseline(self, query, ground_truths=None):
        ...
        if query.limit < 0:
            ...
            for idx in range(nr_non_avgs):
                ...
                if nlenv.is_aggregate(select_str):
                    l = result_l[0][idx]
                    u = result_u[0][idx]
                    if l is None or u is None:
                        lus.append((float('-inf'), float('inf')))
