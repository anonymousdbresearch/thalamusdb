...
from query_info import CardinalityModel
...
class CostOptimizer:
    ...
    def bounds_sum(cardi, col_info):
        ...
        if ratio is None or ratio == 0:
            ...
            n_t = math.ceil(cardi.s_t * ratio * cardi.nr_total)
            ...
            n_tu = math.ceil(cardi.s_tu * ratio * cardi.nr_total)
    ...
    def error_agg(self, cardi, query_info):
        if query_info.agg_func == 'count' or query_info.query.limit >= 0:
            ...
            if query_info.agg_func == 'sum':
                ...
                l_c, u_c = CostOptimizer.bounds_count(cardi, -1)
                l = l_s / u_c
    ...
    def error_feedback(self, action, query_info, nl_filters, f_nrss, c_model):
        ...
        idxs_yes, idxs_no = nl_filters[fid].estimate_idxs_tfu(user_feedback_opt)
        # When user feedback is yes.
        f_nrss_yes = f_nrss.copy()
        f_nrss_yes[fid] = idxs_yes.nrs()
    ...
    def error_actions(self, action2cnt, query_info, nl_filters, f_nrss, c_model):
        ...
        # Constant estimation time for error. We assume 100 thresholds for all predicates.
        nr_total_thresholds = 100
        ...
        for fid, feedback_cnt in fid_to_feedback_cnt.items():
            ...
            threshold_diff = (nl_filter.upper - nl_filter.lower) / (nr_thresholds_per_fid + 1)
            thresholds = [nl_filter.lower + idx * threshold_diff for idx in range(1, nr_thresholds_per_fid + 1)]
            # Find valid splits based on possible thresholds.
            ...
            for split_idx in range(nr_splits):
                ...
                # Equal signs for both inequalities to support corner cases.
                # nr_valids = sum(1 for threshold in thresholds if lower <= threshold <= upper)
                # if nr_valids > 0:
                if any(lower <= threshold <= upper for threshold in thresholds):
                    nrs_per_split.append(nl_filter.idxs_with_temp_bounds(lower, upper).nrs())
        ...
        for nrs_per_fid in generator:
            ...
            for action, cnt in action2cnt_process.items():
                fid = action[-1]
                ...
                if action_type == 'i':
                    f_nrss_estimated[fid] = f_nrss_estimated[fid].estimate(process_percent)
                elif action_type == 'o':
                    (_, _, col_idx, min_max, _) = action
                    col = query_info.query.cols[col_idx]
                    ordering = (min_max, col)
                    f_nrss_estimated[fid] = f_nrss_estimated[fid].estimate(process_percent, ordering)
    ...
    @staticmethod
    def gather_constraint_info(constraint, nr_nl_filters):
        is_error_constraint = constraint[0] == 'error'
        is_feedback_constraint = constraint[0] == 'feedback'
        is_runtime_constraint = constraint[0] == 'runtime'
        assert is_error_constraint or is_feedback_constraint or is_runtime_constraint, f'Unknown constraint: {constraint}'
        error_constraint = constraint[1] if is_error_constraint else None
        feedback_constraint = (nr_nl_filters * constraint[1]) if is_feedback_constraint else None
        runtime_constraint = constraint[1] if is_runtime_constraint else None
        metric_ratio = constraint[2]
        return is_error_constraint, is_feedback_constraint, is_runtime_constraint, error_constraint, feedback_constraint, runtime_constraint, metric_ratio

    def combined_cost(self, runtime, nr_feedbacks, error, constraint_type):
        if constraint_type == 'error':
            cost = runtime + nr_feedbacks * self.metric_ratio
        elif constraint_type == 'feedback':
            cost = runtime + error * self.metric_ratio
        elif constraint_type == 'runtime':
            cost = nr_feedbacks + error * self.metric_ratio
        else:
            raise ValueError(f'Unknown constraint option {constraint_type}')
        return cost

    def all_cost_metrics(self, action2cnt, query_info, nl_filters, f_nrss, c_model, constraint_type):
        ...
        cost = self.combined_cost(runtime, nr_feedbacks, error, constraint_type)
        return runtime, nr_feedbacks, error, cost

    @staticmethod
    def satisfies_constraint(constraint, error, nr_feedbacks, cur_nr_feedbacks, nr_nl_filters, runtime, cur_runtime):
        is_error_constraint, is_feedback_constraint, is_runtime_constraint, error_constraint, feedback_constraint, runtime_constraint, metric_ratio \
            = CostOptimizer.gather_constraint_info(constraint, nr_nl_filters)
        return (is_error_constraint and error <= error_constraint) \
               or (is_feedback_constraint and nr_feedbacks <= (feedback_constraint - cur_nr_feedbacks)) \
               or (is_runtime_constraint and runtime + cur_runtime < runtime_constraint)
    ...
    def optimize_local_search(self, query_info, nl_filters, max_nr_total, constraint, cur_nr_feedbacks, cur_runtime):
        ...
        while True:
            ...
            if accumulated_time > 10:  # or cur_nr_actions >= 10:
                print(f'TIMEOUT - Building sequence: {accumulated_time} {cur_nr_actions}')
                break
            runtime, nr_feedbacks = self.runtime_nr_feedbacks(action2cnt)
            error = self.error_actions(action2cnt, query_info, nl_filters, f_nrss, c_model)
            print(f'{accumulated_time} {cur_nr_actions} {error} {cur_runtime} Building sequence: {[f"{cnt}:{action}~{query_info.query.cols[action[2]]}" if action[0] == "o" else f"{cnt}: {action}" for action, cnt in action2cnt.items()]}')
            if CostOptimizer.satisfies_constraint(constraint, error, nr_feedbacks, cur_nr_feedbacks, len(nl_filters), runtime, cur_runtime):
                break
            ...
            best_actions = []
            for action in CostOptimizer.meaningful_actions(possible_actions, nl_filters, action2cnt):
                modified = action2cnt.copy()
                modified[action] = modified.get(action, 0) + 1
                runtime, nr_feedbacks, error, cost = self.all_cost_metrics(modified, query_info, nl_filters, f_nrss, c_model, constraint_type)
        ...
        while True:
            accumulated_time = time.time() - start_time
            cur_nr_actions = sum(action2cnt.values())
            print(f'{accumulated_time} {cur_nr_actions} {cur_runtime} Current action sequence: {[f"{cnt}:{action}~{query_info.query.cols[action[2]]}" if action[0] == "o" else f"{cnt}: {action}" for action, cnt in action2cnt.items()]}')
            ...
            is_first = False
            prev_runtime, prev_nr_feedbacks, prev_error, prev_cost = self.all_cost_metrics(action2cnt, query_info, nl_filters, f_nrss, c_model, constraint_type)
            prev_not_satisfied = not CostOptimizer.satisfies_constraint(constraint, prev_error, prev_nr_feedbacks, cur_nr_feedbacks, len(nl_filters), prev_runtime, cur_runtime)
            print(f'Current estimated cost metrics: {(prev_runtime, prev_nr_feedbacks, prev_error, prev_cost)}')
            if prev_not_satisfied and constraint_type == 'runtime':
                # Means that none of the action is cheap enough in terms of runtime to be processed.
                break
            is_modified = False
            # 1. Delete an action.
            if cur_nr_actions >= 1:
                nr_to_delete = min(cur_nr_actions, cur_factor)
                for modified in CostOptimizer.delete_k_actions(nr_to_delete, action2cnt):
                    if time.time() - start_time > 100:
                        ...
                        break
                    runtime, nr_feedbacks, error, cost = self.all_cost_metrics(modified, query_info, nl_filters, f_nrss, c_model, constraint_type)
                    if prev_not_satisfied or CostOptimizer.satisfies_constraint(constraint, error, nr_feedbacks, cur_nr_feedbacks, len(nl_filters), runtime, cur_runtime):
                        if cost < prev_cost:
                            action2cnt = modified
                            is_modified = True
                            break
            ...
            if is_modified:
                continue
            # 2. Swap an action.
            nr_to_swap = min(cur_nr_actions, cur_factor)
            if nr_to_swap >= 1:
                for modified in CostOptimizer.swap_k_actions(nr_to_swap, action2cnt, possible_actions):
                    if time.time() - start_time > 100:
                        print(f'TIMEOUT - Local search: {time.time() - start_time}')
                        is_timeout = True
                        break
            ...
            if is_modified:
                continue
            break
        return [action for action, cnt in sorted(action2cnt.items()) for _ in range(cnt)]

    @staticmethod
    def delete_action(action2cnt):
        for key in action2cnt:
            modified = {action: cnt - 1 if action == key else cnt for action, cnt in action2cnt.items() if
                        not (action == key and cnt == 1)}
            yield modified

    @staticmethod
    def add_action(action2cnt, possible_actions):
        for key in possible_actions:
            modified = action2cnt.copy()
            modified[key] = modified.get(key, 0) + 1
            yield modified

    @staticmethod
    def swap_k_actions(k, action2cnt, possible_actions):
        assert k >= 1
        generators = [CostOptimizer.delete_action(action2cnt)]
        for _ in range(k - 1):
            generator = itertools.chain(*generators)
            generators = [CostOptimizer.delete_action(action2cnt) for action2cnt in generator]
        generator = itertools.chain(*generators)
        generators = [CostOptimizer.add_action(action2cnt, possible_actions) for action2cnt in generator]
        for _ in range(k - 1):
            generator = itertools.chain(*generators)
            generators = [CostOptimizer.add_action(action2cnt, possible_actions) for action2cnt in generator]
    ...
    @staticmethod
    def delete_k_actions(k, action2cnt):
        assert k >= 1
        generators = [CostOptimizer.delete_action(action2cnt)]
        for _ in range(k - 1):
            generator = itertools.chain(*generators)
    ...
    @staticmethod
    def add_k_actions(k, action2cnt, possible_actions):
        assert k >= 1
        generators = [CostOptimizer.add_action(action2cnt, possible_actions)]
        for _ in range(k - 1):
            generator = itertools.chain(*generators)
            generators = [CostOptimizer.add_action(action2cnt, possible_actions) for action2cnt in generator]
        return itertools.chain(*generators)
    ...
    def optimize(self, query_info, nl_filters, max_nr_total, look_ahead, constraint, cur_nr_feedbacks, cur_runtime):
        ...
        for nl_filter in nl_filters:
            ...
            print(f'{nl_filter} - # of t, f, u, processed, total: {temp_nrs.t}, {temp_nrs.f}, {temp_nrs.u}, {temp_nrs.processed}, {temp_nrs.nr_total}')
        ...
        action_sets = combinations_with_replacement(possible_actions, look_ahead)
        ...
        if constraint_type == 'error':
            ...
            feedback_constraint = (len(nl_filters) * constraint[1])
            for actions in action_sets:
                action2cnt = {}
                for action in actions:
                    action2cnt[action] = action2cnt.get(action, 0) + 1
        ...
        print(f'Max improvement based on constraint: {constraint_type} {max_improve} {best_action_set} {best_action_sets}')
